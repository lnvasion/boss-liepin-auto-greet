/**
 * database.js — 候选人沟通记录 (猎聘版)
 */
import { logger } from './logger.js';

const DB_KEY = 'liepin_auto_records';
...
class CandidateDatabase {
  ...
  _persist() {
    try { localStorage.setItem(DB_KEY, JSON.stringify(this.records)); } catch (e) {}
  }
  ...
  getAll() { this.load(); return [...this.records]; }
  ...
  download(format = 'csv') {
    ...
    const filename = `liepin-candidates-${new Date().toISOString().slice(0, 10)}.${ext}`;
  ...
  clear() { this.records = []; this._persist(); logger.warn('数据库已清空'); }
}
